...
const OptionWheel = ({
  ...
  const runFrame = useCallback(now => {
    ...
    if (cfg.autoScroll && cfg.loop && !isHovered && !dragRef.current) {
      targetRef.current += cfg.autoScroll * dt;
    }
    ...
    rafRef.current = settled && !cfg.autoScroll ? null : requestAnimationFrame(runFrame);
  }, [isHovered]);
  ...
  }, [items, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, loop, smoothing, autoScroll, applyTarget]);

  useEffect(() => {
    if (autoScroll) startLoop();
  }, [autoScroll, isHovered, startLoop]);
  ...
  return (
    <div
      ...
      className={`option-wheel${side === 'right' ? ' option-wheel--right' : ''}${imageMode ? ' option-wheel--images' : ''}${isDragging ? ' option-wheel--dragging' : ''}${isHovered ? ' option-wheel--hovered' : ''}${className ? ` ${className}` : ''}`}
      style={{
        ...
        >
          {imageMode ? (
            <>
              ...
              <span className="option-wheel__image-label">{label}</span>
            </>
          ) : label}
        </div>
        );
